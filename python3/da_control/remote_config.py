...
def config_para_check(da, file_flash_type, update_file, golden_file):
    ...
    if flash_type == file_flash_type:
        ...
        logging.warning(f'{da.ip} flash type is not correct')
...
def erease_flash(da,flash_addr, filesize):
    ...
    for i in range(15):
        ...
        if(prog_status == 255):
            break
        logging.debug('等待{0}秒'.format(i * 4 + 4))

    logging.debug(f'{da.ip} Erase done')
...
def read_flash(da, flash_addr, target_file_name, filesize):
    ...
    logging.debug(f'{da.ip} read back config data start')
    data1 = da.Read_RAM(flash_addr, filesize)
    logging.debug(f'{da.ip} read back config data end')
...
def da_erase_flash(new_ip, source_file_name, erase=True, is_old_version=False):
    ...
    if(board_status != 1):
        logging.warning(f'{new_ip} 连接失败，请检查')
        return False
    ...
    if config_info_valid == 0:
        logging.warning(f'{new_ip} 配置参数有误，请检查')
        return False
    ...
    if(golden_file == 1 and flash_addr == 0 and flash_type_str == 'SPANSION'):
        ...
        return False
    ######################
    logging.INFO(f'{new_ip} wait 100 second')
    for i in range(100):
        time.sleep(1)
    for i in range(100):
        time.sleep(1)
        if (i % 10) == 0:
            logging.debug(f'{new_ip} 已等待{i}秒：重连设备')
            ...
            if da.connect(new_ip, print_msg = False):
                ...
                end_time = time.time()
                # print(len(data1))
                logging.debug(f'{new_ip} 擦除成功 erase time is{end_time - start_time}')
                return True
...
def da_config_flash(new_ip, source_file_name, erase=False, is_old_version=False):
    ...
    end_time = time.time()
    # print(len(data1))
    logging.info(f'{new_ip} config time is{end_time-start_time}')
...
def config_run(ip, g_file, u_file):
    ...
    if version_check(new_ip):
        ...
        if da_config_flash(new_ip, source_file_name, erase=False):
            ...
            logging.info(f'{ip} 成功{sucess}次，失败{failure}次')
    else:
