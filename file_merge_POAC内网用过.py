...
for ss in dirlist:
    ...
    ftp.cwd(ss)
    print("ftp data directory:", ftp.pwd())
...
if(not os.path.exists(raw_dir)):
    ...
    for ff in matched_files:
        target_file = tmp_dir+ff
        file_handle = open(target_file,"wb").write
        ftp.retrbinary('RETR '+ff, file_handle)
...
dirlist = []
for ss in DRLIST3:
    dirlist.append(DRDATA+ss+'/')
for ss in dirlist:
    tmp_dir = raw_dir+ss[ss.rfind('0B')+3:]
    ...
    try:
        files_in = ftp.nlst()
        real_start, real_stop, matched_files = match_file(files_in, start_time, stop_time, matchcsv=0)
    except:
